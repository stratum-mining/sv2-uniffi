/*
 * Example of how to bootstrap Stratum v2 Extended Channels for Mining Servers.
 *
...
public:
    // static_prefix is a bytes array used to guarantee unique search space
    // allocation across different Mining Servers. It is recommended to use a
    // different static_prefix for each Mining Server.
    MiningServerContext(
        ...
        : extranonce_allocator(sv2::Sv2ExtranonceAllocator::init(
          ...
          expected_shares_per_minute(expected_share_per_minute),
    ...
    sv2::SetNewPrevHashTemplateDistribution cached_set_new_prev_hash_tdp;
    // Whether version rolling is allowed.
    bool version_rolling_allowed;
    // Expected shares per minute.
    float expected_shares_per_minute;
    // Number of successful shares acknowledged in a SubmitShares.Success.
    std::uint32_t share_batch_size;
    // Pool tag to write into scriptSig of the coinbase transaction.
    std::string pool_tag_string;
...
public:
    ConnectionContext(
        ...
        // template revenue. This is also where merged mining OP_RETURNs should
        // be added.
    ...
    bool requires_version_rolling;
    // Extended channels established with this specific client.
    std::map<std::uint32_t, std::shared_ptr<sv2::Sv2ExtendedChannelServer>> extended_channels;
    // Standard channels established with this specific client. This is not
...
std::vector<sv2::Sv2Message> bootstrap_extended_channel_server(
    MiningServerContext &mining_server,
    ConnectionContext &connection,
    const sv2::OpenExtendedMiningChannel &open_extended_mining_channel_message
) {
    /*
    ...
    std::shared_ptr<sv2::Sv2ExtranoncePrefix> extranonce_prefix;

    try {
        extranonce_prefix = mining_server.extranonce_allocator->allocate_extended(
            open_extended_mining_channel_message.min_extranonce_size
        );
    ...
    std::shared_ptr<sv2::Sv2ExtendedChannelServer> new_extended_channel;

    try {
        new_extended_channel = sv2::Sv2ExtendedChannelServer::init(
            connection.next_channel_id(),
            open_extended_mining_channel_message.user_identity,
            extranonce_prefix,
            open_extended_mining_channel_message.max_target,
            open_extended_mining_channel_message.nominal_hash_rate,
            mining_server.version_rolling_allowed,
            CLIENT_SEARCH_SPACE_BYTES,
            mining_server.share_batch_size,
            mining_server.expected_shares_per_minute,
            mining_server.pool_tag_string
        );
    } catch (const sv2::sv2_server_extended_channel_error::InvalidNominalHashrate &) {
        return {
            sv2::Sv2Message(
                sv2::Sv2Message::kOpenMiningChannelError{
                    .message = sv2::OpenMiningChannelError{
    ...
    } catch (const sv2::sv2_server_extended_channel_error::RequestedMinExtranonceSizeTooLarge &) {
        return {
            sv2::Sv2Message(
                sv2::Sv2Message::kOpenMiningChannelError{
                    .message = sv2::OpenMiningChannelError{
    ...
    } catch (const sv2::Sv2ServerExtendedChannelError &) {
        return {
            sv2::Sv2Message(
                sv2::Sv2Message::kOpenMiningChannelError{
                    .message = sv2::OpenMiningChannelError{
    ...
    if (new_extended_channel == nullptr) {
        return {
            sv2::Sv2Message(
                sv2::Sv2Message::kOpenMiningChannelError{
                    .message = sv2::OpenMiningChannelError{
    ...
    const auto group_channel_id = connection.group_channel->get_group_channel_id();
    connection.group_channel->add_channel_id(
        new_extended_channel->get_channel_id(),
        new_extended_channel->get_full_extranonce_size()
    );

    const auto open_extended_mining_channel_success =
        sv2::OpenExtendedMiningChannelSuccess{
            .request_id = open_extended_mining_channel_message.request_id,
    ...
    // the OpenExtendedMiningChannel message.
    std::vector<sv2::Sv2Message> ret_messages{
        sv2::Sv2Message(
            sv2::Sv2Message::kOpenExtendedMiningChannelSuccess{
                .message = open_extended_mining_channel_success,
            }
        ),
    };
    ...
    if (connection.requires_work_selection) {
        // Bootstrap the channel state with the cached SetNewPrevHash.
        new_extended_channel->on_set_new_prev_hash(
            mining_server.cached_set_new_prev_hash_tdp
        );

        // Only send the OpenExtendedMiningChannelSuccess message to the client.
        // Do not send a future job or SetNewPrevHashMining.
        return ret_messages;
    }
    ...
    const auto future_extended_job_id =
        new_extended_channel->get_future_job_id_from_template_id(
            mining_server.cached_future_template.template_id
        );

    if (!future_extended_job_id.has_value()) {
        throw std::runtime_error("future extended job id was not cached");
    }

    // We are going to send the future job to the client.
    const auto future_extended_job_message =
        new_extended_channel->get_future_job(*future_extended_job_id)
            ->get_job_message();
    ...
    ret_messages.push_back(
        sv2::Sv2Message(
            sv2::Sv2Message::kNewExtendedMiningJob{
                .message = future_extended_job_message,
            }
        )
    );
    ret_messages.push_back(
...
std::string pretty_format(
    const std::shared_ptr<sv2::Sv2ExtendedChannelServer> &channel
) {
    std::ostringstream oss;
    oss << "Sv2ExtendedChannelServer(channel_id=" << channel->get_channel_id() << ")";
    return oss.str();
}
...
std::string pretty_format(const sv2::OpenExtendedMiningChannelSuccess &message) {
    std::ostringstream oss;
    oss
...
std::string pretty_format(const sv2::NewExtendedMiningJob &message) {
    std::ostringstream oss;
    oss
    ...
    oss
        ...
        << ", version_rolling_allowed=" << (message.version_rolling_allowed ? "True" : "False")
        << ", merkle_path=[" << message.merkle_path.size() << " items]"
        << ", coinbase_tx_prefix=" << pretty_bytes(message.coinbase_tx_prefix)
        << ", coinbase_tx_suffix=" << pretty_bytes(message.coinbase_tx_suffix)
        << ")";
    return oss.str();
}
...
std::string pretty_format(const sv2::Sv2Message &message) {
    ...
    if (std::holds_alternative<sv2::Sv2Message::kOpenExtendedMiningChannelSuccess>(variant)) {
        return pretty_format(
            std::get<sv2::Sv2Message::kOpenExtendedMiningChannelSuccess>(variant).message
        );
    }
    ...
    if (std::holds_alternative<sv2::Sv2Message::kNewExtendedMiningJob>(variant)) {
        return pretty_format(
            std::get<sv2::Sv2Message::kNewExtendedMiningJob>(variant).message
        );
    }
...
int main() {
    try {
        std::cout << "Stratum v2 Extended Channel Server Example" << std::endl;
        ...
        // Imagine client A sends an OpenExtendedMiningChannel message. Notice
        // that they are requesting a min_extranonce_size of 8, which is allowed
        // by the mining server.
        const auto open_extended_mining_channel_message_a_1 =
            sv2::OpenExtendedMiningChannel{
                .request_id = 1,
                .user_identity = "A1",
        ...
        const auto ret_a_1 = bootstrap_extended_channel_server(
            mining_server_context,
            connection_a,
            open_extended_mining_channel_message_a_1
        );

        print_response_messages(
            "OpenExtendedMiningChannel message a_1",
            ret_a_1
        );

        // Imagine client A sends another OpenExtendedMiningChannel message.
        // Notice that they are requesting a min_extranonce_size of 17, while
        // the mining server only allows CLIENT_SEARCH_SPACE_BYTES (16) at most.
        const auto open_extended_mining_channel_message_a_2 =
            sv2::OpenExtendedMiningChannel{
                .request_id = 1,
                .user_identity = "A1",
                .nominal_hash_rate = 1'000'000.0F,
        ...
        const auto ret_a_2 = bootstrap_extended_channel_server(
            mining_server_context,
            connection_a,
            open_extended_mining_channel_message_a_2
        );

        print_response_messages(
            "OpenExtendedMiningChannel message a_2",
            ret_a_2
        );

        // Imagine client B sends an OpenExtendedMiningChannel message.
        const auto open_extended_mining_channel_message_b =
            sv2::OpenExtendedMiningChannel{
                .request_id = 3,
                .user_identity = "B1",
                .nominal_hash_rate = 1'000'000.0F,
        ...
        const auto ret_b = bootstrap_extended_channel_server(
            mining_server_context,
            connection_b,
            open_extended_mining_channel_message_b
        );

        print_response_messages(
            "OpenExtendedMiningChannel message b",
            ret_b
        );

        // Imagine client B sends another OpenExtendedMiningChannel message.
        const auto open_extended_mining_channel_message_b_2 =
            sv2::OpenExtendedMiningChannel{
                .request_id = 4,
                .user_identity = "B2",
                .nominal_hash_rate = 1'000'000.0F,
        ...
        const auto ret_b_2 = bootstrap_extended_channel_server(
            mining_server_context,
            connection_b,
            open_extended_mining_channel_message_b_2
        );

        print_response_messages(
            "OpenExtendedMiningChannel message b_2",
            ret_b_2
        );
        ...
        std::cout << std::string(50, '=') << std::endl;
        std::cout << std::string(50, '=') << std::endl;
        std::cout << "connection_a.extended_channels:" << std::endl;
        for (const auto &[_, channel] : connection_a.extended_channels) {
            (void)_;
            std::cout << pretty_format(channel) << std::endl;
        }
        std::cout << std::string(50, '-') << std::endl;
        std::cout
            << "connection_a.group_channel: "
            << pretty_format(connection_a.group_channel)
            << std::endl;

        std::cout << std::string(50, '=') << std::endl;
        std::cout << std::string(50, '=') << std::endl;
        std::cout << "connection_b.extended_channels:" << std::endl;
        for (const auto &[_, channel] : connection_b.extended_channels) {
            (void)_;
            std::cout << pretty_format(channel) << std::endl;
        }
        std::cout << std::string(50, '-') << std::endl;
        std::cout
            << "connection_b.group_channel: "
            << pretty_format(connection_b.group_channel)
            << std::endl;
